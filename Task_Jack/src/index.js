...
function main(sources) {
  ...
  const code$ = xs
    .fromPromise(
      fetch("/fsms/sandbox.txt", {
        headers: {
          "content-type": "text/plain"
        }
      })
    )
    .map(v => xs.fromPromise(v.text()))
    .flatten();

  const goal$ = xs
    .combine(
      code$,
      sources.DOM.select("button#start").events("click", {
        preventDefault: true
      })
    )
    .map(([code, click]) => code);

  const sinks = withRobotActions(Robot, { hidePoseViz: true })({
    ...sources,
    goal: goal$
  });
