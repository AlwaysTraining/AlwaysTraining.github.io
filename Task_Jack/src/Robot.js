...
function interp(tree) {
  if (tree.type === "started") {
    ...
    return (prev, input) =>
      prev.s === interp(tree.value[0]) &&
      input.type === interp(tree.value[1]).type
        ? {
            ...prev,
...
function input(goal$, speechSynthesisResult$, twoSpeechbubblesResult$) {
  return xs.merge(
    goal$.map(g => ({ type: "goal", value: g })),
    speechSynthesisResult$
      .filter(r => r.status.status === Status.SUCCEEDED)
      .mapTo({ type: "speakDone" })
      .compose(delay(200)),
...
function output(reducerState$) {
  const outputs$ = reducerState$.filter(m => !!m.o).map(m => m.o);
  return {
